...
/**********************
 This function compiles assembler files.
 It takes as input a string with the name of the assembler 
 file to load. Once loaded, the file is compiled into an object
 file and saved with the same name as the assembler file but 
 with a ".s" extension.
**********************/
int Assembler::assemble(string asmFile)
{
  ...
  //create name of object file 
  objFile.replace( asmFile.find(".s"), 2, ".o" );

  // open new file to be saved as object file
  ...
  // main, parsing/compiling loop
  getline( infile, lineOfCode);
  while( !infile.eof() ){
	  ...
	  if( instr_set.count(opcode) > 0 )  //make sure instruction is in map
	      ...
	      cout << "Error. In line:" << LineNumber << endl
		   << "Invalid Instruction:  " << lineOfCode << endl;
	      otherError = true;
	  }
	  RD = RS = -1;
	  CONST = 129;
